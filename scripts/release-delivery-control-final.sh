...
OUT_SCOPE="$(sg diff --name-only "$MAIN_TIP" "$DEPLOY_SHA" | grep -Ev '^(delivery-control/|packages/delivery-shared/|docs/|scripts/|backend/tests/|backend/src/services/(capabilities\.js|expenseSubmission/service\.js)$)' || true)"
[ -z "$OUT_SCOPE" ] || die "berkas di luar scope Delivery Control: ${OUT_SCOPE}"
ok "seluruh perubahan Delivery Control berada di delivery-control/, packages/delivery-shared/, docs/, scripts/, tests, dan dua berkas backend di atas"
