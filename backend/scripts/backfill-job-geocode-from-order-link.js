...
// dipakai untuk geocoding job sampai fix 7 September 2026 ini.
//
...
// masih null. Script ini untuk kasus yang self-heal TIDAK menyentuh: job yang
// SUDAH punya lat/lng (dari Nominatim/LocationIQ, mungkin kurang akurat) —
// ensureJobsGeocoded() SENGAJA tidak menimpa job yang sudah ada koordinat
// (supaya klik "Buat Peta" tetap cepat, tidak fetch ulang utk stop yang
// sebenarnya sudah OK). Script ini yang melakukan upgrade retroaktif itu:
// SEMUA job aktif yang order-nya punya locationUrl di-geocode ULANG dari
// link itu dan MENIMPA lat/lng lama, apa pun sumbernya sebelumnya — link
// Order.locationUrl dianggap paling akurat, konsisten dengan prioritas yang
// sama di geocodeAddress().
//
...
import { geocodeFromMapsLink } from "../src/services/maps.js";
...
async function main() {
  ...
  for (const j of jobs) {
    ...
    let geo;
    try {
      geo = await geocodeFromMapsLink(j.order.locationUrl);
    } catch (err) {
      console.log(`${label}: GAGAL fetch link (${err.message}) — dilewati.`);
      gagalResolve++;
      continue;
    }
    if (!geo) {
