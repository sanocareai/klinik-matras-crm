// KALIBRASI SALDO RIIL Kas & Bank per 19 Sep 2026 20.00 WIB — SATU jurnal koreksi (source SALDO_AWAL) via ledger resmi.
// Aturan & alasan waktu: lihat kepala services/finance/kalibrasiSaldo.js.
//
//   node scripts/kalibrasiSaldoRiil20260919.js           # PRATINJAU (tidak menulis apa pun)
//   KALIBRASI_BACKUP_OK=1 node scripts/kalibrasiSaldoRiil20260919.js --apply
//
...
import { KALIBRASI_20260919 as K, hitungPosisi, hitungKoreksi, postKalibrasi } from "../src/services/finance/kalibrasiSaldo.js";
...
async function main() {
  ...
  const posisi = await hitungPosisi(prisma);
  const koreksi = hitungKoreksi(posisi);
  ...
  if (!APPLY) { console.log("\nIni PRATINJAU. Backup dulu, lalu: KALIBRASI_BACKUP_OK=1 node scripts/kalibrasiSaldoRiil20260919.js --apply"); return; }
  ...
  const hasil = await prisma.$transaction((tx) => postKalibrasi(tx, { userId: admin.id }), { timeout: 60_000, maxWait: 20_000 });
  ...
  const sesudah = await hitungPosisi(prisma);
  ...
  const ulang = await prisma.$transaction((tx) => postKalibrasi(tx, { userId: admin.id }));
