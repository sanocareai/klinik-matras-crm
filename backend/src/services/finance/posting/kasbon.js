...
/**
 * Pemotongan kasbon dari gaji lewat modul Kasbon (FinKasbonRepayment):
 *
 *   Dr Beban Gaji (6-1100)   Cr Piutang Karyawan (1-1350)     — kas TIDAK tersentuh
 *
 * Kasbon adalah gaji yang dicairkan lebih awal, jadi tidak pernah "dikembalikan"
 * — hanya dipotong. Kenapa Beban Gaji, bukan Utang Gaji seperti
 ...
 * mengkreditnya.
 */
export async function postKasbonPelunasan(tx, { repaymentId, date, amount, karyawanNama, userId = null }) {
  ...
  const { entry, created } = await postJournal(tx, {
    date,
    description: `Potongan kasbon dari gaji — ${nama}`,
    source: "KASBON",
    sourceId: repaymentId,
    idempotencyKey: key,
    userId,
    lines: [
      { accountId: bebanGaji.id, debit: nominal, description: `Potongan kasbon dari gaji — ${nama}` },
      { accountId: piutangKaryawan.id, credit: nominal, description: `Pemotongan kasbon — ${nama}` },
    ],
  });
  return { posted: true, entry, created };
}
