...
export function nextBestAction(s) {
  ...
  // `prospectStalled` (lihat signals.js), digeneralisasi dari "sudah
  // di-quote tapi diam" jadi "macet di PROSPECT terlalu lama".
  if (s.prospectStalled)
    return { action: "Follow up prospek", reason: "Sudah beberapa hari tanpa progres, belum direspons lanjut.", urgency: "high" };
  ...
  if (s.stage === "PROSPECT" && s.orderCount === 0)
    return { action: "Tawarkan rekomendasi kasur", reason: "Prospek aktif, belum ada order.", urgency: "medium" };
