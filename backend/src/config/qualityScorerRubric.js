...
// ── Tool schema utk native structured output (Anthropic tool use) ─────────
// (28 Agustus 2026, fix bug JSON-escaping ~5-7%) — GANTI dari "minta LLM
// menulis JSON di teks bebas lalu JSON.parse manual" (rawan gagal kalau ada
// tanda kutip TIDAK ter-escape di dalam value "quote", karena decoding teks
// bebas tidak menjamin escaping benar) ke tool_choice PAKSA + input_schema.
// Anthropic men-generate `input` lewat constrained decoding sesuai skema
// dan mengembalikannya API SUDAH sebagai objek (bukan string yang perlu
// di-parse manual) — ini "native JSON mode" versi Anthropic. 0 dependency
// baru: endpoint & fungsi (chatWithTools) yang sama, sudah dipakai fitur
// co-pilot KB, cuma belum pernah dipakai grading.js.
//
// Efek SAMPING yang diinginkan: `additionalProperties:false` + `required`
// pada level tiap objek dimensi membuat skema TIDAK PUNYA slot key di root
// utk field seperti "objectionType"/flag boolean — jadi bug lama "LLM taruh
// field di root, bukan bersarang" (lihat catatan panjang di job.js#dimResult)
// jadi TIDAK MUNGKIN terjadi lagi secara struktural, bukan cuma "diharapkan
// tidak terjadi lewat instruksi teks". dimResult() di job.js TETAP menjaga
// fallback top-level lama sebagai jaring pengaman (harmless kalau tidak
// pernah kepakai lagi), TIDAK dihapus di perubahan ini.
export const QUALITY_SCORER_TOOL_NAME = "submit_penilaian_kualitas";
...
export function buildRubricTool() {
  const properties = { overallNote: { type: "string" } };
  const required = ["overallNote"];
  for (const d of RUBRIC_DIMENSIONS) {
    properties[d.key] = dimensionSchema(d);
    required.push(d.key);
  }
  return {
    name: QUALITY_SCORER_TOOL_NAME,
    description:
      "Kirim hasil penilaian kualitas percakapan sales sesuai rubrik SANO Care Sales Framework — field score/quote/strength/weakness (+field tambahan tiap dimensi kalau ada) HARUS bersarang di dalam objek dimensinya masing-masing.",
