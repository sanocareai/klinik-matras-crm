...
export default function ChartTooltip({ active, payload, label, formatter, labelFormatter }) {
  ...
  return (
    <div className="rounded-xl bg-surface/95 px-3.5 py-2.5 shadow-popover shadow-slate-900/10 backdrop-blur-sm">
      {label != null && (
        ...
        {payload.map((entry, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2 text-xs">
              ...
              <span className="ml-auto font-semibold text-ink">{val}</span>
            </div>
          );
        })}
      </div>
