...
export const api = {
  ...
  // Insentif per alamat (D-162, 13 September 2026). `patch` bisa
  // { hasSim } dan/atau { isFreelance } — PATCH /armada/drivers/:id
  // menerima keduanya opsional.
  ...
  unsubscribePush: (endpoint) => request("/armada/push/unsubscribe", { method: "POST", body: JSON.stringify({ endpoint }) }),
  getPayments: (params = {}) => {
  ...
  createFinanceKasbon: (data) => request("/finance/kasbon", { method: "POST", body: JSON.stringify(data) }),
  catatPelunasanKasbon: (id, data) => request(`/finance/kasbon/${id}/pelunasan`, { method: "POST", body: JSON.stringify(data) }),
  potongKasbonKaryawan: (data) => request("/finance/kasbon/pelunasan-karyawan", { method: "POST", body: JSON.stringify(data) }),
  ...
  getUangMuka: (params = {}) => request(`/finance/uang-muka${qsFinance(params)}`),
  getUangMukaRiwayat: () => request("/finance/uang-muka/riwayat"),
  berikanUangMuka: (data) => request("/finance/uang-muka", { method: "POST", body: JSON.stringify(data) }),
  pertanggungjawabanUangMuka: (id, data) => request(`/finance/uang-muka/${id}/pertanggungjawaban`, { method: "POST", body: JSON.stringify(data) }),
  kembalikanUangMuka: (id, data) => request(`/finance/uang-muka/${id}/kembalikan`, { method: "POST", body: JSON.stringify(data) }),
  batalkanPengembalianUangMuka: (id, sid, reason) => request(`/finance/uang-muka/${id}/pengembalian/${sid}/batal`, { method: "POST", body: JSON.stringify({ reason }) }),
  batalkanUangMuka: (id, reason) => request(`/finance/uang-muka/${id}/batal`, { method: "POST", body: JSON.stringify({ reason }) }),
  getUangMukaAktifPengajuan: (params = {}) => request(`/finance/expense-submissions/uang-muka-aktif${qsFinance(params)}`),
  ...
  getFinancePemasukanRingkasan: (params = {}) => request(`/finance/pemasukan/ringkasan${qsFinance(params)}`),
  getFinancePemasukan: (params = {}) => request(`/finance/pemasukan${qsFinance(params)}`),
  getFinancePemasukanOpsi: () => request("/finance/pemasukan/opsi"),
  getFinancePemasukanDetail: (jenis, id) => request(`/finance/pemasukan/${jenis}/${id}`),
  getLegacyCutoff: () => request("/finance/pemasukan/legacy/cutoff"),
  getLegacyBatches: () => request("/finance/pemasukan/legacy/batch"),
  getLegacyBatch: (id, params = {}) => request(`/finance/pemasukan/legacy/batch/${id}${qsFinance(params)}`),
  uploadLegacyBatch: (formData) => requestFormData("/finance/pemasukan/legacy/batch", formData),
  imporLegacyBatch: (id) => request(`/finance/pemasukan/legacy/batch/${id}/impor`, { method: "POST", body: JSON.stringify({}) }),
  batalLegacyBatch: (id, alasan) => request(`/finance/pemasukan/legacy/batch/${id}/batal`, { method: "POST", body: JSON.stringify({ alasan }) }),
  keputusanLegacyBaris: (id, keputusan) => request(`/finance/pemasukan/legacy/baris/${id}/keputusan`, { method: "POST", body: JSON.stringify({ keputusan }) }),
  getLegacyRekonsiliasi: () => request("/finance/pemasukan/legacy/rekonsiliasi"),
  getLegacyProposal: () => request("/finance/pemasukan/legacy/proposal"),
