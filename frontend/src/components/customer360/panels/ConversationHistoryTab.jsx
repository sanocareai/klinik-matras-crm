...
export default function ConversationHistoryTab({ conversations, loading, error, onRetry, onOpenChat }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {convos.map((conv) => (
        <div key={conv.id} className="rounded-xl p-3">
          <div className="mb-2 flex items-center gap-2 text-[11px]">
            <Badge variant="success">{conv.channel === "WHATSAPP" ? "WhatsApp" : conv.channel}</Badge>
            <span className="text-ink3">{formatTanggalWaktu(conv.lastMessageAt)}</span>
          ...
          <div className="flex flex-col gap-1">
            {(conv.messages || []).slice(-8).map((m) => (
              <div key={m.id} className={`max-w-[85%] rounded-lg px-2.5 py-1.5 text-[12px] ${m.direction === "OUTBOUND" ? "self-end bg-accentbg text-accent" : "self-start bg-inset text-ink"}`}>
                {m.content || (m.mediaType ? `[${m.mediaType}]` : "")}
                <div className="mt-0.5 text-[9.5px] text-ink3">{formatWaktu(m.createdAt)}</div>
              </div>
            ))}
            {(conv.messages || []).length === 0 && <p className="text-[12px] text-ink3">Belum ada pesan.</p>}
          </div>
        </div>
      ))}
    </div>
