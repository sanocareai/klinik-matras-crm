...
export default function DateRangePicker({ value, onChange }) {
  ...
  const inputCls = "h-8 w-full rounded border border-slate-200 px-2 text-[12px] tabular-nums text-slate-700 " +
    "focus-visible:outline-none focus-visible:border-brand-500 focus-visible:ring-2 focus-visible:ring-brand-600/30";
  ...
  return (
    <div ref={rootRef} className="relative flex items-center gap-1">
      ...
      <button
        ...
        className={cn(
          "flex h-9 items-center gap-2 rounded-lg border bg-white px-3 text-left transition-colors duration-150",
          "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600/40",
          open ? "border-brand-500" : "border-slate-200 hover:border-slate-300"
        )}
      >
        <Calendar size={14} className="shrink-0 text-slate-400" />
        <span className="text-[11px] font-medium text-slate-400">{presetLabel(value)}</span>
        <span className="text-[13px] font-semibold tabular-nums text-slate-800">{formatRangeText(value)}</span>
      ...
      <button
        ...
        className="grid h-8 w-7 place-items-center rounded-lg border border-slate-200 bg-white text-slate-500 transition-colors hover:bg-slate-50 disabled:pointer-events-none disabled:opacity-40"
      >
        <ChevronLeft size={15} />
      </button>
      <button
      ...
      {open && (
        <div
          ...
          className="absolute right-0 top-11 z-50 flex w-[min(92vw,620px)] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg sm:flex-row"
        >
          {/* ── KIRI: preset ── */}
          <div className="flex max-h-[62vh] shrink-0 flex-col overflow-y-auto border-slate-100 sm:max-h-[440px] sm:w-[210px] sm:border-r">
            <button
              type="button"
              onClick={() => { setDraft({ ...draft, preset: "custom" }); setPicking(false); }}
              className={cn(
                "px-4 py-2 text-left text-[13px] transition-colors",
                draft?.preset === "custom" ? "bg-brand-50 font-semibold text-brand-700" : "text-slate-600 hover:bg-slate-50"
              )}
            >
              Kustom
            </button>
            ...
            {SIMPLE_PRESETS.map((p) => (
              <button
                ...
                className={cn(
                  "px-4 py-2 text-left text-[13px] transition-colors",
                  draft?.preset === p.id ? "bg-brand-50 font-semibold text-brand-700" : "text-slate-600 hover:bg-slate-50"
                )}
              >
                {p.label}
              </button>
            ...
            <div className="mt-1 flex flex-col gap-1.5 border-t border-slate-100 px-3 py-2.5">
              {PARAM_PRESETS.map((p) => (
                <div key={p.id} className="flex items-center gap-2">
                  <input
                    ...
                    className="h-7 w-12 rounded border border-slate-200 px-1.5 text-center text-[12px] tabular-nums focus-visible:outline-none focus-visible:border-brand-500"
                    aria-label={p.label}
                  />
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      draft?.preset === p.id ? "font-semibold text-brand-700" : "text-slate-500 hover:text-slate-700"
                    )}
                  >
                    {p.label}
                  </button>
            ...
            <label className="mt-auto flex cursor-pointer items-center justify-between gap-2 border-t border-slate-100 px-4 py-3">
              <span className="text-[13px] text-slate-600">Bandingkan</span>
              <span className="relative inline-flex">
                ...
                />
                <span className="h-5 w-9 rounded-full bg-slate-200 transition-colors peer-checked:bg-brand-600 peer-focus-visible:ring-2 peer-focus-visible:ring-brand-600/40" />
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <div className="flex items-end gap-2 border-b border-slate-100 px-3.5 py-3">
              <label className="min-w-0 flex-1">
                <span className="mb-1 block text-[10px] font-medium text-slate-400">Tanggal mulai*</span>
                <input
              ...
              <label className="min-w-0 flex-1">
                <span className="mb-1 block text-[10px] font-medium text-slate-400">Tanggal akhir*</span>
                <input
                  type="date" className={inputCls}
                  value={draft?.to || ""}
                  max={todayWIB().format("YYYY-MM-DD")}
            ...
            <div className="flex items-center justify-between px-3.5 py-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className="grid h-6 w-6 place-items-center rounded text-slate-500 hover:bg-slate-100 disabled:pointer-events-none disabled:opacity-30"
                >
                  <ChevronRight size={15} />
                </button>
              </div>
            ...
            <div className="mt-auto flex items-center justify-between gap-2 border-t border-slate-100 px-3.5 py-2.5">
