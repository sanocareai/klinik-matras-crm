...
export function PinPad({ judul, sub, pesan, onSelesai, sibuk, nonaktif, biometrik }: Props) {
  ...
  const tombol = (isi: React.ReactNode, onPress: () => void, label: string, testID: string, tampil = true) => (
    <PressableScale
      ...
      style={{ width: 72, height: 72, borderRadius: 36, alignItems: "center", justifyContent: "center", backgroundColor: tampil ? colors.glassFillStrong : "transparent", borderWidth: tampil ? 1 : 0, borderColor: colors.glassStroke }}
    >
  ...
  return (
    <View style={{ alignItems: "center", width: "100%" }}>
      <Text accessibilityRole="header" style={{ color: colors.text, fontFamily: font.semibold, fontSize: 22, textAlign: "center" }}>{judul}</Text>
      {sub ? <Text style={{ color: colors.textMuted, fontFamily: font.regular, fontSize: 14, lineHeight: 20, textAlign: "center", marginTop: 6 }}>{sub}</Text> : null}

      <View style={{ flexDirection: "row", gap: 14, marginTop: 26, marginBottom: 14 }} accessibilityLabel={`${pin.length} dari ${PIN_LENGTH} digit terisi`} accessibilityLiveRegion="polite">
      ...
      <View style={{ gap: 14 }}>
        {BARIS.map((baris) => (
          <View key={baris.join("")} style={{ flexDirection: "row", gap: 22 }}>
